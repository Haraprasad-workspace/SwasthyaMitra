  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FFFBF5] flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <div className="font-heading text-xl text-[#967A53]">Accessing Command Center...</div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex min-h-screen bg-[#FFFBF5] font-body text-[#422D0B]">
      ...
      <div className="flex-grow flex flex-col h-screen overflow-y-auto">
        <nav className="bg-white border-b border-[#E8DDCB] px-8 py-4 shadow-sm sticky top-0 z-30 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <div className={`w-3 h-3 rounded-full ${isSyncing ? 'bg-[#FFA800] animate-ping' : 'bg-green-500'}`}></div>
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-[#967A53]">Operational Status</p>
        ...
        <main className="flex-grow max-w-7xl w-full mx-auto px-8 py-10">
          ...
          <div className="grid lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 bg-white border border-[#E8DDCB] p-10 rounded-[3.5rem] shadow-sm relative overflow-hidden group">
              ...
              <div className="h-72 flex items-end justify-between gap-4 px-2 mb-4">
                {[30, 45, 25, 60, stats.activeQueue * 8 + 10, 35, 20].map((h, i) => (
                  <div key={i} className="flex-grow bg-[#FFFBF5] rounded-t-3xl relative group/bar overflow-hidden" style={{ height: `${Math.min(h + 15, 100)}%` }}>
                    <div className={`absolute inset-0 transition-all duration-700 ${i === 4 ? 'bg-[#FFA800]' : 'bg-[#E8DDCB] group-hover/bar:bg-[#FFA800]/40'}`}></div>
                  </div>
                ))}
              </div>
              <div className="flex justify-between mt-8 text-[10px] font-black uppercase text-[#967A53] px-6 tracking-[0.4em] opacity-60">
            ...
            <div className="space-y-8">
              <div className="bg-white border border-[#E8DDCB] p-8 rounded-[3rem] shadow-sm">
                <div className="flex items-center gap-3 mb-6">
                  <UserCheck size={20} className="text-[#FFA800]" />
...
const InsightCard = ({ label, value, icon, color, pulse }) => (
  <div className={`bg-white border border-[#E8DDCB] p-8 rounded-[3rem] shadow-sm transition-all hover:shadow-xl hover:border-[#FFA800] group ${pulse ? 'ring-4 ring-red-500/10' : ''}`}>
    ...
    <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#967A53] mb-1 group-hover:text-[#422D0B]">{label}</p>
    <p className="text-4xl font-heading text-[#422D0B]">{value}</p>
  </div>
);
