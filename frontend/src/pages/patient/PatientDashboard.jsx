...
const PatientDashboard = () => {
  ...
  if (loading) return (
    <div className="min-h-screen bg-[#FFFBF5] flex flex-col items-center justify-center">
      ...
      <p className="font-heading text-[#422D0B]">Unlocking Health Vault...</p>
    </div>
  );

  return (
    <div className="min-h-screen bg-[#FFFBF5] font-body text-[#422D0B] flex flex-col">
      <nav className="bg-white border-b border-[#E8DDCB] px-6 py-4 sticky top-0 z-30 shadow-sm flex justify-between items-center">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-[#FFA800] rounded-xl flex items-center justify-center text-white font-heading text-xl shadow-md cursor-pointer" onClick={() => fetchProfile(true)}>S</div>
          <div>
      ...
      <main className="flex-grow max-w-7xl w-full mx-auto px-6 py-10">
        <header className="bg-[#422D0B] text-[#FFFBF5] p-10 rounded-[3rem] shadow-xl mb-12 flex flex-col md:flex-row justify-between items-center relative overflow-hidden">
          ...
          <ShieldCheck size={100} className="text-[#FFA800] opacity-10 absolute right-10 bottom-[-20px] md:bottom-auto" />
        </header>

        <div className="grid lg:grid-cols-4 gap-12">
          <div className="lg:col-span-1 space-y-8">
            <div className="bg-white border border-[#E8DDCB] p-8 rounded-[3rem] shadow-sm">
              <h3 className="font-heading text-xl mb-6 flex items-center gap-2"><Activity className="text-[#FFA800]" size={20} /> Snapshot</h3>
            ...
            <div className="bg-white border border-[#E8DDCB] p-8 rounded-[3rem] shadow-sm sticky top-32">
          ...
          <div className="lg:col-span-3 space-y-8">
            ...
            {filteredHistory.length > 0 ? (
              filteredHistory.map((visit) => {
                ...
                return (
                  <div key={visit._id} className="bg-white p-8 rounded-[3.5rem] border border-[#E8DDCB] shadow-sm hover:border-[#FFA800] transition-all animate-in fade-in slide-in-from-bottom-2">
                    ...
                    <div className="bg-[#FFFBF5] p-6 rounded-[2rem] border border-[#E8DDCB]/50 mb-8 font-medium text-[#422D0B] text-sm line-clamp-2">
...
const VitalMini = ({ label, val, unit }) => (
  <div className="flex justify-between items-end border-b border-[#E8DDCB]/30 pb-3 mb-3">
    <span className="text-[10px] font-black text-[#967A53] uppercase tracking-widest">{label}</span>
    <div className="text-right">
      <span className="text-xl font-heading text-[#422D0B]">{val || '0'}</span>
      <span className="text-[8px] font-bold text-[#967A53] ml-1">{unit}</span>
    </div>
  </div>
);
