...
const PatientCheckIn = () => {
  ...
  const handleSendOTP = async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/auth/patient/send-otp', { 
  ...
  const handleVerifyAndCheckin = async (e) => {
    ...
    try {
      // 1. Verify OTP first
      await axios.post('http://localhost:5000/api/auth/patient/verify-otp', {
        phone: formData.patientPhone,
        otp
      });

      // 2. Submit the Gatekeeper Check-in Request
      const res = await axios.post('http://localhost:5000/api/queue/public/checkin', {
  ...
  return (
    <div className="min-h-screen bg-[#FFFBF5] font-body text-[#422D0B] flex flex-col items-center justify-center p-6">
      ...
      <p className="mt-8 text-[9px] font-black uppercase tracking-[0.4em] text-[#967A53] opacity-50">
        Powered by Swasthya-Mitra Clinical Network
      </p>
    </div>
  );
};
